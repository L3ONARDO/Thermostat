...
public class HeatingSystem {
    ...
    public static WeekProgram getWeekProgram() throws ConnectException,
        ...
        try {
            ...
            WeekProgram program = new WeekProgram(tag_values[0], tag_values[1], tag_values[2], tag_values[3], tag_values[4], tag_values[5], tag_values[6]);
            ...
            while (parser.next() != XmlPullParser.END_DOCUMENT) {
                ...
                if (parser.getName().equals("day")) {
                    ...
                    program.setData(current_day, switches.get(0), switches.get(1), false);
                }
            }
            return program;
     ...
     * @throws IOException
     */
    private static ArrayList<ArrayList<Integer>> parseSwitches(XmlPullParser parser)
            throws XmlPullParserException, IOException {
        ArrayList<ArrayList<Integer>> switches = new ArrayList<ArrayList<Integer>>();
        ArrayList<Integer> daySwitches = new ArrayList<Integer>();
        ArrayList<Integer> nightSwitches = new ArrayList<Integer>();

        while (parser.next() != XmlPullParser.END_DOCUMENT) {
            int eventType = parser.getEventType();
            if (eventType == XmlPullParser.END_TAG
                    && parser.getName().equals("day")) {
                switches.add(daySwitches);
                switches.add(nightSwitches);
                return switches;
            }
            ...
            if (parser.getName().equals("switch")) {
                ...
                if (type.equalsIgnoreCase("day")) {
                    daySwitches.add(front_int * 100 + back_int);
                } else {
                    nightSwitches.add(front_int * 100 + back_int);
                }
            }
        }
        switches.add(daySwitches);
    ...
    public static void put(String attribute_name, String value)
        ...
        String link = "";
        boolean match = false;
        String[] valid_names = { "day", "time", "targetTemperature",
                "dayTemperature", "nightTemperature", "weekProgramState" };
        for (int i = 0; i < valid_names.length; i++) {
            if (attribute_name.equalsIgnoreCase(valid_names[i])) {
                match = true;
                if (attribute_name.equalsIgnoreCase("targetTemperature")) {
                    link = HeatingSystem.BASE_ADDRESS + "/currentTemperature";
                } else {
                    link = HeatingSystem.BASE_ADDRESS + "/" + valid_names[i];
                }
